...
class UserController extends Controller
{
    ...
    public function changePassword(Request $request){
        ...
        if (!(Hash::check($request->get('current-password'), Auth::user()->userPassword))) {
            ...
            return redirect()->back()->with("passerror","Your current password does not matches with the password you provided. Please try again.")->with('passErrorModal', 1);
        }

        if(strcmp($request->get('current-password'), $request->get('new-password')) == 0){
            //Current password and new password are same
            return redirect()->back()->with("passerror","New Password cannot be same as your current password. Please choose a different password.")->with('passErrorModal', 1);
        }
